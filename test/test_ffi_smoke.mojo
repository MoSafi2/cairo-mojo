...
def test_cairo_version_is_positive() raises:
    var handle = CairoLoader.ensure_loaded()
...
def test_image_surface_lifecycle_smoke() raises:
    ...
    var surface = cairo_image_surface_create(
        format, c_int(16), c_int(16)
    )
    var surface_status = cairo_surface_status(surface)
    assert_equal(surface_status.value, cairo_status_t.CAIRO_STATUS_SUCCESS.value)
...
def test_context_draw_smoke() raises:
    ...
    var format = cairo_format_t(c_int(0))
    var surface = cairo_image_surface_create(
        format, c_int(32), c_int(32)
    )
    assert_equal(
        cairo_surface_status(surface).value, cairo_status_t.CAIRO_STATUS_SUCCESS.value
    )

    var ctx = cairo_create(surface)
    assert_equal(cairo_status(ctx).value, cairo_status_t.CAIRO_STATUS_SUCCESS.value)
...
def test_extended_ffi_entrypoints_smoke() raises:
    ...
    assert_equal(cairo_surface_status(surface).value, cairo_status_t.CAIRO_STATUS_SUCCESS.value)
    ...
    cairo_set_antialias(ctx, materialize[cairo_antialias_t.CAIRO_ANTIALIAS_BEST]())
    cairo_set_line_cap(ctx, materialize[cairo_line_cap_t.CAIRO_LINE_CAP_ROUND]())
    cairo_set_line_join(ctx, materialize[cairo_line_join_t.CAIRO_LINE_JOIN_BEVEL]())
    ...
    cairo_surface_mark_dirty_rectangle(surface, c_int(0), c_int(0), c_int(8), c_int(8))
...
def test_pattern_and_text_ffi_entrypoints_smoke() raises:
    ...
    cairo_pattern_set_extend(pattern, materialize[cairo_extend_t.CAIRO_EXTEND_REPEAT]())
    cairo_pattern_set_filter(pattern, materialize[cairo_filter_t.CAIRO_FILTER_BILINEAR]())
    assert_equal(cairo_pattern_status(pattern).value, cairo_status_t.CAIRO_STATUS_SUCCESS.value)
    ...
    var family_ptr = (
        family_mut.as_c_string_slice().unsafe_ptr().unsafe_origin_cast[ImmutExternalOrigin]()
    )
    cairo_select_font_face(
        ctx,
        family_ptr,
    ...
    var metrics_ptr = (
        metrics_mut.as_c_string_slice().unsafe_ptr().unsafe_origin_cast[ImmutExternalOrigin]()
    )
